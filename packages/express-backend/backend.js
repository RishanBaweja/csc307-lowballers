...
app.get("/items/:id", async (req, res) => {
  try {
    ...
    const item = await itemServices.findItemById(id);
...
app.get("/conversation", async (req, res) => {
  try {
    const conversations = await messageServices.getInboxForUser(req.user._id);
...
// Start (or find) a conversation for an item + other user
...
app.post(
  "/me/profile-picture",
  requireAuth,
  upload.single("profilePicture"), // field name in the form
  async (req, res) => {
    try {
      if (!req.file) {
